  ...
  const id = req.params.id;
   await db.findById(id)
    .then((user: User) => {
      ...
      res.status(200).json(`User : ${user.firstName} ${user}`)
    })
    .catch(() => {
      res.status(500).json({ message: `Canot find this id=${id}`});
...
router.post("/", async (req:Request, res:Response) => {
  ...
  try {
    Db = await Db.save();    
res.status(201).json(`User ${user.firstName} Created :ID :`+ Db._id)
  } catch (err) {
...
router.patch("/:id", getUsers, async (req:Request, res:any) => {
  ...
  try {
    ...
    res.json(`User ${updateUser.firstName} Update Details :` + updateUser);
  } catch (err) {
